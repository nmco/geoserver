...
public final class HiveFilterToSql extends PreparedFilterToSQL {
    ...
    protected Object visitBinarySpatialOperatorz(
        ...
        try {
            // TODO handle swaped
            /**
             * e1 = clipToWorld(filter, e1); e2 = clipToWorld(filter, e2);
             *
             * <p>if (filter instanceof Beyond || filter instanceof DWithin) doSDODistance(filter,
             * e1, e2, extraData); else if (filter instanceof BBOX && looseBBOXEnabled) {
             * doSDOFilter(filter, e1, e2, extraData); } else doSDORelate(filter, e1, e2, swapped,
             * extraData);*
             */
            ...
            out.write(
                    String.format(
                            ...
                            bbox.getBounds().getMinX(),
                            bbox.getBounds().getMaxX(),
                            bbox.getBounds().getMinY(),
